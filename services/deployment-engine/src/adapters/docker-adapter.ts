...
const DOCKER_BUILD_TIMEOUT_MS = env.ENGINE_BUILD_TIMEOUT_SECONDS * 1000;
...
function nodeSourceRootLines(): string[] {
  return [
    ...
    '      if [ -z "$second" ]; then printf "%s" "$first"; return; fi; \\',
    '      preferred="$(printf "%s\\n" "$candidates" | grep -E "^(apps|services|web|app|server|backend|frontend|api)(/|$)" | sed -n "1p")"; \\',
    '      if [ -n "$preferred" ]; then printf "%s" "$preferred"; return; fi; \\',
    '      printf "%s" "$first"; \\',
    '    }; \\',
    '    source_dir="/repo"; \\',
    ...
    '        source_dir="/repo"; \\',
    '      else \\',
    '        auto_dir="$(detect_package_dir)"; \\',
    '        if [ -n "$auto_dir" ]; then \\',
    '          echo ">>> Requested root directory ${requested} does not contain package.json; auto-detected ${auto_dir}"; \\',
    '          resolved="$auto_dir"; \\',
    '          source_dir="/repo/$resolved"; \\',
    '        else \\',
    '          echo ">>> Unable to resolve Node app root automatically. Checked ${requested} and repository root for package.json. Continuing with repository root."; \\',
    '          resolved="."; \\',
    '          source_dir="/repo"; \\',
    '        fi; \\',
    '      fi; \\',
...
function staticSourceRootLines(): string[] {
  return [
    '# Resolve configured root directory before later COPY steps',
    'ARG ROOT_DIR=.',
    'RUN set -eu; \\',
    ...
    '    has_static_marker() { \\',
    '      dir="$1"; \\',
    '      [ -f "$dir/index.html" ] || [ -f "$dir/public/index.html" ]; \\',
    '    }; \\',
    '    detect_static_dir() { \\',
    '      package_candidates="$(find /repo \\',
    '        -path /repo/.git -prune -o \\',
    '        -path "*/node_modules" -prune -o \\',
    '        -name package.json -print | sed "s#^/repo/##" | while read -r file; do dirname "$file"; done | sort -u)"; \\',
    '      first_package="$(printf "%s\\n" "$package_candidates" | sed -n "1p")"; \\',
    '      second_package="$(printf "%s\\n" "$package_candidates" | sed -n "2p")"; \\',
    '      if [ -n "$first_package" ] && [ -z "$second_package" ]; then printf "%s" "$first_package"; return; fi; \\',
    '      preferred_package="$(printf "%s\\n" "$package_candidates" | grep -E "^(apps|services|web|app|server|backend|frontend|api|site|client)(/|$)" | sed -n "1p")"; \\',
    '      if [ -n "$preferred_package" ]; then printf "%s" "$preferred_package"; return; fi; \\',
    '      if [ -n "$first_package" ]; then printf "%s" "$first_package"; return; fi; \\',
    '      static_candidates="$(find /repo \\',
    '        -path /repo/.git -prune -o \\',
    '        -path "*/node_modules" -prune -o \\',
    '        -name index.html -print | sed "s#^/repo/##" | while read -r file; do dirname "$file"; done | sort -u)"; \\',
    '      first_static="$(printf "%s\\n" "$static_candidates" | sed -n "1p")"; \\',
    '      second_static="$(printf "%s\\n" "$static_candidates" | sed -n "2p")"; \\',
    '      if [ -n "$first_static" ] && [ -z "$second_static" ]; then printf "%s" "$first_static"; return; fi; \\',
    '      preferred_static="$(printf "%s\\n" "$static_candidates" | grep -E "^(apps|services|web|app|server|backend|frontend|api|site|client)(/|$)" | sed -n "1p")"; \\',
    '      if [ -n "$preferred_static" ]; then printf "%s" "$preferred_static"; return; fi; \\',
    '      printf "%s" "$first_static"; \\',
    '    }; \\',
    '    source_dir="/repo"; \\',
    '    [ "$resolved" = "." ] || source_dir="/repo/$resolved"; \\',
    '    if ! has_package_json "$source_dir" && ! has_static_marker "$source_dir"; then \\',
    '      if has_package_json "/repo" || has_static_marker "/repo"; then \\',
    '        echo ">>> Requested root directory ${requested} does not contain a static app root; falling back to repository root"; \\',
    '        resolved="."; \\',
    '        source_dir="/repo"; \\',
    '      else \\',
    '        auto_dir="$(detect_static_dir)"; \\',
    '        if [ -n "$auto_dir" ]; then \\',
    '          echo ">>> Requested root directory ${requested} does not contain a static app root; auto-detected ${auto_dir}"; \\',
    '          resolved="$auto_dir"; \\',
    '          source_dir="/repo/$resolved"; \\',
    '        else \\',
    '          echo ">>> Unable to resolve static app root automatically. Continuing with repository root."; \\',
    '          resolved="."; \\',
    '          source_dir="/repo"; \\',
    '        fi; \\',
    '      fi; \\',
...
function webServiceDockerfile(projectId: string): string {
  return [
    ...
    'RUN --mount=type=bind,from=source,source=/apployd-target,target=/src \\',
    '    [ -f /src/package.json ] && cp /src/package.json ./package.json || true; \\',
    ...
    '# Restore root monorepo config files for apps that extend ../../tsconfig.base.json',
    'RUN --mount=type=bind,from=source,source=/repo,target=/repo-source \\',
    '    set -e; \\',
    '    for f in tsconfig.base.json tsconfig.json jsconfig.json; do \\',
    '      if [ -f "/repo-source/$f" ]; then cp "/repo-source/$f" "/$f"; fi; \\',
    '    done',
    '',
    ...
    '        echo ">>> Runtime bind host defaults: HOST=${HOST:-0.0.0.0} HOSTNAME=${HOSTNAME:-0.0.0.0}"; \\',
    '        printf \'#!/bin/sh\\nHOST="${HOST:-0.0.0.0}" HOSTNAME="${HOSTNAME:-0.0.0.0}" exec npm run start:prod\\n\' > /entrypoint.sh; \\',
    '      elif [ -n "$START" ] && ! is_dev_cmd "$START"; then \\',
    '        echo ">>> Using: npm start"; \\',
    '        echo ">>> Runtime bind host defaults: HOST=${HOST:-0.0.0.0} HOSTNAME=${HOSTNAME:-0.0.0.0}"; \\',
    '        printf \'#!/bin/sh\\nHOST="${HOST:-0.0.0.0}" HOSTNAME="${HOSTNAME:-0.0.0.0}" exec npm start\\n\' > /entrypoint.sh; \\',
    '      elif [ -n "$START_SERVE" ] && ! is_dev_cmd "$START_SERVE"; then \\',
    '        echo ">>> Using: npm run serve"; \\',
    '        echo ">>> Runtime bind host defaults: HOST=${HOST:-0.0.0.0} HOSTNAME=${HOSTNAME:-0.0.0.0}"; \\',
    '        printf \'#!/bin/sh\\nHOST="${HOST:-0.0.0.0}" HOSTNAME="${HOSTNAME:-0.0.0.0}" exec npm run serve\\n\' > /entrypoint.sh; \\',
    '      elif [ -n "$MAIN" ]; then \\',
    ...
    '      echo ">>> Detected static site content"; \\',
    '      if [ -f public/index.html ] && [ ! -f index.html ]; then \\',
    "        printf '#!/bin/sh\\ncd /app/public\\nexec python3 -m http.server ${PORT:-3000} --bind 0.0.0.0\\n' > /entrypoint.sh; \\",
    '      else \\',
    "        printf '#!/bin/sh\\ncd /app\\nexec python3 -m http.server ${PORT:-3000} --bind 0.0.0.0\\n' > /entrypoint.sh; \\",
    '      fi; \\',
...
function staticSiteDockerfile(projectId: string): string {
  return [
    ...
    '    else \\',
    '      FOUND=""; \\',
    '      if [ -f "/app/index.html" ]; then FOUND="."; fi; \\',
    '      if [ -z "$FOUND" ] && [ -f "/app/public/index.html" ]; then FOUND="public"; fi; \\',
    '      for d in build/static build dist out public/build .next/standalone .output/public; do \\',
    '        if [ -z "$FOUND" ] && [ -d "/app/$d" ]; then FOUND="$d"; break; fi; \\',
    '      done; \\',
    '      if [ -z "$FOUND" ]; then \\',
    '        echo "WARNING: No output directory detected, serving repository root"; \\',
    '        FOUND="."; \\',
    '      fi; \\',
