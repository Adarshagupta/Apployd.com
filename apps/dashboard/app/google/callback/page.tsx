...
function GoogleCallbackContent() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      if (!code) {
        setState('error');
        setMessage('Missing Google login code. Please try signing in again.');
        return;
      }
      ...
      try {
        ...
        if (!token) {
          throw new Error('Google login exchange did not return a session token.');
        }
  ...
  return <GoogleCallbackCard state={state} message={message} />;
}

function GoogleCallbackCard({ state, message }: { state: CallbackState; message: string }) {
  return (
    <main className="grid min-h-screen place-items-center px-4">
      <article className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 text-center shadow-sm">
        {state === 'processing' ? (
          ...
          <>
            <h1 className="text-lg font-semibold text-slate-900">Google sign-in failed</h1>
            <p className="mt-2 text-sm text-slate-600">{message}</p>
            <div className="mt-4">
...
export default function GoogleCallbackPage() {
  return (
    <Suspense fallback={<GoogleCallbackCard state="processing" message="Finalizing Google sign-in..." />}>
